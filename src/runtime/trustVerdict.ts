...
export type TrustVerdictStatus = "trusted" | "review_required" | "blocked";
export type TrustCheckStatus = "passed" | "review_required" | "blocked";
export type TrustPolicyAction = "allow" | "review" | "block";

export interface TrustPolicyRule {
  action?: TrustPolicyAction;
  requireRationale?: boolean;
  missingRationaleAction?: Exclude<TrustPolicyAction, "allow">;
  requireTrustedProducer?: boolean;
...
export interface TrustPolicy {
  ...
  policyId: string;
  sourceLabel: string;
  requireTrustedProducerForTrusted: boolean;
  defaultUnsignedAction: Exclude<TrustPolicyAction, "allow">;
  defaultPolicyWarningAction: Exclude<TrustPolicyAction, "allow">;
  defaultHighRiskAllowAction: Exclude<TrustPolicyAction, "allow">;
  defaultMissingRationaleAction: Exclude<TrustPolicyAction, "allow">;
  rulePolicies: Record<string, TrustPolicyRule>;
}

export interface TrustVerdictCheck {
  name: string;
...
export interface TrustVerdictArtifact {
  schemaVersion: 1;
  kind: "wutai.trust_verdict";
  taskId: string;
  generatedAt: string;
  verdict: TrustVerdictStatus;
  summary: string;
  policy: {
    policyId: string;
    sourceLabel: string;
...
function policyAction(value: unknown): TrustPolicyAction | undefined {
  return value === "allow" || value === "review" || value === "block"
    ? value
    : undefined;
}

function reviewOrBlock(
  value: unknown,
  fallback: Exclude<TrustPolicyAction, "allow">,
): Exclude<TrustPolicyAction, "allow"> {
  return value === "block" || value === "review" ? value : fallback;
}

export function normalizeTrustPolicy(
  raw: unknown,
  sourceLabel = "local trust policy",
): TrustPolicy {
...
function statusForAction(action: Exclude<TrustPolicyAction, "allow">): TrustCheckStatus {
...
export function buildTrustVerdictArtifact({
  ...
  policy?: PolicyLike | null;
  integrity?: IntegrityLike | null;
  provenance?: ProvenanceLike | null;
  policyReview?: PolicyReviewLike | null;
  trustPolicy?: TrustPolicy;
}): TrustVerdictArtifact {
  const checks: TrustVerdictCheck[] = [];
  ...
  if (provenance?.status === "passed") {
    addCheck(
      ...
      "passed",
      "packet_provenance",
      provenance.summary ?? "Packet provenance passed.",
    );
  ...
  if (policyReview?.status === "passed") {
    addCheck(
      ...
      policyReview.summary ?? "Policy override review passed.",
    );
    ...
    addCheck(
      ...
      "blocked",
      "policy_override_review",
      policyReview.summary ?? "Policy override review failed.",
    );
  } else {
    addCheck(
      checks,
  ...
  if (policy?.decision === "deny") {
    ...
    addCheck(
      ...
      "blocked",
      "policy_decision",
      "The packet policy decision is missing.",
    );
  }
  ...
  return {
    ...
    inputs: {
      ...
      producerAdapter:
        manifest?.producer?.adapter ?? provenance?.manifest?.producerAdapter,
