...
| External trace import / CLI wrapper | Local-script trace import, developer CLI wrapper with small policy preflight, UI packet review, manifest hash check, and audit detail browsing in v0.2 foundation | Extend later to Claude Code, Codex, or other agent traces after the work-packet schema is stable. |
...
- CLI wrapper policy/provenance expansion or coding-agent trace importer for one external workflow.
