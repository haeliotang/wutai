...
| External trace import / CLI wrapper | Local-script trace import and developer CLI wrapper with small policy preflight in v0.2 foundation | Extend later to Claude Code, Codex, or other agent traces after the work-packet schema is stable. |
...
- CLI wrapper policy expansion or coding-agent trace importer for one external workflow.
