...
- developer CLI wrapper 会对显式传入的本地命令做 rule-based policy preflight，按决策执行或拒绝，并生成 `manifest.json`、`report.md`、`policy.json`、`trace.json`、`ledger.json` 和 `audit.json`，记录 argv、cwd、policy decision、exit code、stdout/stderr 摘要和 git-status delta。
...
下一目标：扩展 CLI wrapper policy preflight、packet provenance check 和 audit filtering，同时继续保持 schema 可表示 coding-agent session 和 browser task。
...
- CLI wrapper policy preflight：更完整的危险命令 warning、explicit allow/deny、policy artifact
...
- Wutai 支持通过开发期 CLI wrapper 对显式传入的本地命令做 rule-based policy preflight，执行或拒绝，并生成本地 work packet。
