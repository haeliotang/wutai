...
- developer CLI wrapper 会对显式传入的本地命令做 structured policy preflight，从 `config/wutai-cli-policy-profiles.json` 或 `--policy-config` 读取 profile 行为，支持 `standard` / `strict` profile、`--dry-run` review 和可选 `--signing-key` packet attestation，按决策执行、warning、override、pending review 或拒绝，并生成 `manifest.json`、`report.md`、`policy.json`、`trace.json`、`ledger.json`、`audit.json` 以及可选 `attestation.json`，记录 argv、cwd、policy profile、execution mode、policy decision、rule category、default action、review scope、exit code、stdout/stderr 摘要和 git-status delta。
...
下一目标：推进 trusted-key enrollment UX / revoked-key regression、外部 rule override、或一个 coding-agent trace source，同时继续保持 schema 可表示 coding-agent session 和 browser task。
...
- CLI wrapper policy preflight：外部 rule override、更多 rule regression
- CLI wrapper packet review：trusted-key enrollment UX、revoked-key regression、signature tamper regression
...
- Wutai 支持通过开发期 CLI wrapper 对显式传入的本地命令做 structured policy preflight，从外部 JSON 配置读取 policy profile 行为，支持 `standard` / `strict` profile、`--dry-run` review 和可选 `--signing-key` packet attestation，执行、warning、override、pending review 或拒绝，并生成本地 work packet。
