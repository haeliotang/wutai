...
- developer CLI wrapper 会对显式传入的本地命令做 structured policy preflight，支持 `standard` / `strict` profile 和 `--dry-run` review，按决策执行、warning、override、pending review 或拒绝，并生成 `manifest.json`、`report.md`、`policy.json`、`trace.json`、`ledger.json` 和 `audit.json`，记录 argv、cwd、policy profile、execution mode、policy decision、rule category、default action、review scope、exit code、stdout/stderr 摘要和 git-status delta。
- desktop/web UI 能导入 CLI wrapper packet 目录或文件，把它转成本地 task history，复算 manifest artifact hash，生成 `integrity.json`，展示 policy、trace、ledger、audit 和 integrity review surface，并对 dry-run pending execution 记录本地 approve/deny `review.json`。

下一目标：把 CLI policy profile 配置外部化，推进 packet provenance check 和 audit filtering，同时继续保持 schema 可表示 coding-agent session 和 browser task。
...
- CLI wrapper policy preflight：外部化 profile 配置、桌面确认流、更多 rule regression
- CLI wrapper packet review：更强 provenance check、artifact 签名边界、audit filtering
...
- Wutai 支持通过开发期 CLI wrapper 对显式传入的本地命令做 structured policy preflight，支持 `standard` / `strict` profile 和 `--dry-run` review，执行、warning、override、pending review 或拒绝，并生成本地 work packet。
- Wutai 支持在 UI 中导入 CLI wrapper packet 目录或文件，校验 manifest artifact hash，review policy、trace、ledger、audit、integrity，并为 dry-run pending execution 记录本地 approve/deny。
...
- CLI wrapper packet 会生成 `manifest.json`、`report.md`、`policy.json`、`trace.json`、`ledger.json`、`audit.json`；UI 导入后会额外保存 `integrity.json`，dry-run review 决策会额外保存 `review.json`。
