...
- MCP tool-call trace importer 会把一个已运行 MCP session 的声明式 trace 转成 `mcp_tool_call` work packet，记录 server/tool、request summary、argument preview、result summary、resource、credential purpose、audit trail 和 artifact hash，并明确标记未 proxy、未执行、未批准/拦截 MCP tool call。
- local file ingestion 会把用户显式选择的本地文件转成 `local_file` work packet，记录 metadata、SHA-256、bounded preview、file-read audit 和 artifact hash，并明确标记未 crawl 目录、未 watch 后续变化、未保留完整文件内容。
...
下一目标：推进 trusted-key enrollment UX、更多 rule override regression、MCP trace/local file negative-case validation，或定义最小 credential-broker boundary，同时继续保持 schema 可表示 coding-agent session 和 browser task。
...
- MCP trace import：negative-case validation 和 schema fixture
- local file ingestion：retention policy、size limit、hash re-check UX
...
- Wutai 支持 ingest 用户显式选择的本地文件，并生成 `local_file` work packet，保留 metadata、SHA-256 和 bounded preview。
...
- local file packet 会生成 `manifest.json`、`report.md`、`files.json`、`audit.json`。
