...
当前仓库实现一个受监督的研究任务链路，并新增一个 local-script trace-import 楔子，用它们证明以下本地信任层闭环：
...
当前未实现任意外部 agent 的运行期强制监督、shell command 执行代理、MCP proxy、浏览器控制、Codex/Claude Code 适配、完整 computer-use、跨 agent credential broker、移动端确认器和生产打包。
...
1. 基于当前 trace importer 的 CLI wrapper。
...
下一目标：把 trace import 推进为真正的 CLI wrapper，同时继续保持 schema 可表示 coding-agent session 和 browser task。
...
目标：把当前 local-script trace importer 推进成一个低风险的运行期监督入口。
...
- Wutai 已经能执行或沙箱任意 shell command。
