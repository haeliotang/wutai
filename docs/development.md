...
下一目标：扩展 CLI wrapper policy preflight，并让桌面端能导入或 review CLI wrapper packet，同时继续保持 schema 可表示 coding-agent session 和 browser task。
...
- CLI wrapper packet review：桌面端导入 `artifacts/cli/<session_id>/manifest.json`
