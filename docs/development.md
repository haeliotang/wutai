...
- desktop/web UI 能导入 CLI wrapper packet 目录或文件，把它转成本地 task history，复算 manifest artifact hash，生成 `integrity.json` 和 `provenance.json`，展示 policy、trace、ledger、filtered audit、integrity 和 provenance review surface，校验可选 `attestation.json` 对当前 `manifest.json` 的 ECDSA P-256 SHA-256 签名，并根据用户显式加载或本地 enrollment 的 trusted-producer policy 判断 key 是否可信；verified-but-unknown key 可由用户显式 enroll 到本地 policy 并立刻重算 provenance；dry-run pending execution 可记录本地 approve/deny `review.json`。

下一目标：推进 trusted-key revoke/edit UX、更多 rule override regression，或定义最小 credential-broker boundary，同时继续保持 schema 可表示 coding-agent session 和 browser task。
...
- CLI wrapper packet review：trusted-key revoke/edit UX
...
- Wutai 支持在 UI 中导入 CLI wrapper packet 目录或文件，校验 manifest artifact hash，记录 packet provenance，校验可选 packet attestation signature，并用显式加载或本地 enrollment 的 trusted-producer policy 判断 key 是否可信；同时 review policy、trace、ledger、filtered audit、integrity、provenance，并为 dry-run pending execution 记录本地 approve/deny。
