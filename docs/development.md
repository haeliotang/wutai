...
- desktop/web UI 能导入 CLI wrapper packet 目录或文件，把它转成本地 task history，复算 manifest artifact hash，生成 `integrity.json` 和 `provenance.json`，展示 policy、trace、ledger、filtered audit、integrity 和 provenance review surface，校验可选 `attestation.json` 对当前 `manifest.json` 的 ECDSA P-256 SHA-256 签名，并对 dry-run pending execution 记录本地 approve/deny `review.json`。

下一目标：推进 trusted-key registry / producer trust policy、外部 rule override、或一个 coding-agent trace source，同时继续保持 schema 可表示 coding-agent session 和 browser task。
...
- CLI wrapper packet review：trusted-key registry、producer trust policy、signature tamper regression
...
- Wutai 支持在 UI 中导入 CLI wrapper packet 目录或文件，校验 manifest artifact hash，记录 packet provenance，校验可选 packet attestation signature，review policy、trace、ledger、filtered audit、integrity、provenance，并为 dry-run pending execution 记录本地 approve/deny。
