...
- desktop/web UI 能导入 CLI wrapper packet 文件，把它转成本地 task history，并展示 policy、trace、ledger 和 audit review surface。

下一目标：扩展 CLI wrapper policy preflight 和 packet review 深度，同时继续保持 schema 可表示 coding-agent session 和 browser task。
...
- CLI wrapper packet review：目录级导入、artifact 完整性检查、audit 详情浏览
...
- Wutai 支持在 UI 中导入 CLI wrapper packet 文件并 review policy、trace、ledger、audit。
...
- CLI wrapper packet 会生成 `manifest.json`、`report.md`、`policy.json`、`trace.json`、`ledger.json`、`audit.json`。
