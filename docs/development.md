...
- desktop/web UI 能导入 CLI wrapper packet 目录或文件，把它转成本地 task history，复算 manifest artifact hash，生成 `integrity.json`，并展示 policy、trace、ledger、audit 和 integrity review surface。

下一目标：把 CLI policy profile 外部化，连接 dry-run review 到桌面确认流，推进 packet provenance check 和 audit filtering，同时继续保持 schema 可表示 coding-agent session 和 browser task。
...
- Wutai 支持在 UI 中导入 CLI wrapper packet 目录或文件，校验 manifest artifact hash，并 review policy、trace、ledger、audit、integrity。
...
- CLI wrapper packet 会生成 `manifest.json`、`report.md`、`policy.json`、`trace.json`、`ledger.json`、`audit.json`；UI 导入后会额外保存 `integrity.json`。
