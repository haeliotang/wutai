...
当前仓库实现一个受监督的研究任务链路、一个 local-script trace-import 楔子、一个开发期 CLI wrapper、External Agent Integration Contract、Agent Packet Inbox，以及 v0.10 permission-basis typed attention gate，用它们证明以下本地信任层闭环：
...
- **默认本地可审计**：任务、事件、权限、成果物、证据结果和限定范围签收结果应优先落入本地 ledger。
...
- **人类背书不可自动化**：agent 可以准备 review surface，但不能替用户做最终 alignment / accountability 判断；无 `declaredScope` / `excludedScope` 的签收是 theater anti-signal。
- **自动放行只吃机械依据**：`auto_accepted_under_policy` 必须有 grant-eligible `permissionBasis`；语义比对、模型判断和 model-backed external check 只能升级 attention，不能授予许可。
...
src/
  ...
  runtime/
    agentPacketInbox.ts           Agent Packet Inbox 派生索引、filter 和 summary
    adapterRegistry.ts            UI 侧 adapter registry
    cliPacketImporter.ts          CLI wrapper packet 文件导入和 task 转换
    ...
    localScriptTraceImporter.ts   local-script trace import 楔子
...
scripts/
  ...
  wutai_attention_gate.mjs        attention decision gate，把 verified packet 路由到 auto/human/scoped/block 四态，并拆分 permission basis / risk signals
...
examples/
  ...
  consumer-attestation.example.json
                                  v0.7 scoped ratification example
  review-session.example.json     v0.8 attention/causal-credit session example
...
config/
  ...
  wutai-attention-policy.example.json
                                  v0.10 attention policy example
...
tests/
  ...
  node/wutai_attention_gate.test.mjs
                                  v0.10 attention decision gate 回归
...
| CLI wrapper | `scripts/wutai_run.mjs` | 开发期 local command 执行 ledger 和 policy preflight |
...
Scoped ratification gate CLI smoke：
...
Attention decision gate CLI smoke：

```bash
npm run wutai:attention -- --help
npm run wutai -- attention-decision --help
```
...
1. 基于当前 CLI wrapper 的更完整 policy preflight 和桌面端 review。
...
- research workflow 会生成 `manifest.json`，并记录 artifact inventory、SHA-256、权限摘要、Evidence Gate 状态、coverage 和 blind spots。
- local-script trace importer 会把一个已运行命令的声明式 trace 转成 `manifest.json`、`report.md`、`trace.json` 和 `audit.json`，并明确标记未执行、未沙箱、未强制权限的边界。
- coding-agent trace importer 会把一个已运行外部 coding-agent session 的声明式 trace 转成 `coding_agent` work packet，记录 tool call、file change、credential purpose、runtime summary、audit trail 和 artifact hash，并明确标记未执行、未监督、未强制权限的边界。
- MCP tool-call trace importer 会把一个已运行 MCP session 的声明式 trace 转成 `mcp_tool_call` work packet，记录 server/tool、request summary、argument preview、result summary、resource、credential purpose、audit trail 和 artifact hash，并在创建 task 前拒绝缺失 tool name、非法 status/timestamp、负 latency 和过量 tool-call batch；同时明确标记未 proxy、未执行、未批准/拦截 MCP tool call。
- local file ingestion 会把用户显式选择的本地文件转成 `local_file` work packet，记录 metadata、SHA-256、bounded preview、file-read audit 和 artifact hash，限制 batch/file size，并支持重新选择文件生成 `file-check.json` 做 hash/size re-check；同时明确标记未 crawl 目录、未 watch 后续变化、未保留完整文件内容。
- developer CLI wrapper 会对显式传入的本地命令做 structured policy preflight，从 `config/wutai-cli-policy-profiles.json` 或 `--policy-config` 读取 profile 行为，支持 `standard` / `strict` profile、profile 内 rule-level overrides、`--dry-run` review 和可选 `--signing-key` packet attestation，按决策执行、warning、override、pending review 或拒绝，并生成 `manifest.json`、`report.md`、`policy.json`、`trace.json`、`ledger.json`、`audit.json` 以及可选 `attestation.json`，记录 argv、cwd、policy profile、execution mode、policy decision、rule category、default action、effective action、rule override、review scope、exit code、stdout/stderr 摘要和 git-status delta。
- desktop/web UI 能导入 CLI wrapper packet 目录或文件，把它转成本地 task history，复算 manifest artifact hash，生成 `integrity.json` 和 `provenance.json`，展示 policy、trace、ledger、filtered audit、integrity 和 provenance review surface，校验可选 `attestation.json` 对当前 `manifest.json` 的 ECDSA P-256 SHA-256 签名，并根据用户显式加载或本地 enrollment 的 trusted-producer policy 判断 key 是否可信；verified-but-unknown key 可由用户显式 enroll 到本地 policy 并立刻重算 provenance；本地 trust registry 可查看、revoke、reactivate、export key，并会对当前 packet 立刻重算 provenance；dry-run pending execution 可记录本地 approve/deny `review.json`。
...
- Attention Decision Gate 会读取 packet directory，重新运行 packet verifier，可选读取 `consumer-attestation-check.json` 和 `wutai.attention_policy`，输出 `attention-decision.json`；v0.9 会把 packet 路由到 `auto_accepted_under_policy`、`human_attention_required`、`scoped_ratified`、`blocked_or_unowned`，并在没有 accepted scoped ratification 时显式记录 `no_human_review` audit signal。v0.10 增加 `permissionBasis` / `riskSignals` 拆分：自动放行必须引用 grant-eligible 的机械或 deterministic external check basis；model-backed external check、语义比对和模型判断只能作为 attention/risk signal，不能授予放行；内置默认 policy 要求 auto acceptance 前存在 accountable seat，且缺 seat 会无条件记录 audit reason。该 gate 不证明静默 human review、不证明 reviewer identity、不自动读取 GitHub PR metadata 或 CODEOWNERS、不独立证明外部检查的 determinism、不证明 trace 完整性、不提供 runtime sandbox。

下一目标：并行做一次真实 policy-owner 对话和一次真实 agent PR / packet 的 attention-decision dry run，验证 owner 是否愿意为机械条件下的 auto-accept policy 背书，以及 `auto_accepted_under_policy`、`human_attention_required`、`blocked_or_unowned` 是否能解释实际 review load。代码层只能继续推进验证仪器：policy health dry run、owner extraction fixture、GitHub check 输出、或标准 provenance 输出；不要把 v0.10 的 gate 叙述成已验证产品。

验收标准：

- schema 已能表示 research、coding-agent session、browser task、local script。
- 每个 manifest artifact inventory 有 stable id、type、path、created_at、producer、hash。
- audit 已能表示 tool call、permission decision、credential purpose、runtime event。
- evidence section 已能保留 claims、sources、unsupported items、blind spots。
- README 和 technical design 必须继续明确哪些 runtime 已支持，哪些只是 schema 预留。

### 16.2 External Agent Supervision Wedge

目标：把当前 CLI wrapper 推进成更接近 permission broker 的低风险运行期监督入口。

优先候选：

- CLI wrapper policy preflight：更多 rule override regression 和 config validation
- CLI wrapper packet review：local review artifact hardening
- MCP trace import：更多 schema fixture 和 live proxy boundary definition
- local file ingestion：hash re-check review surface hardening
...
- Wutai 支持导入一个 local-script trace，并生成本地 work packet。
- Wutai 支持导入一个 coding-agent trace，并生成 `coding_agent` work packet。
- Wutai 支持导入一个 MCP tool-call trace，并生成 `mcp_tool_call` work packet。
- Wutai 支持 ingest 用户显式选择的本地文件，并生成 `local_file` work packet，保留 metadata、SHA-256 和 bounded preview；重新选择文件后可生成 `file-check.json` 做 hash/size re-check。
- Wutai 支持通过开发期 CLI wrapper 对显式传入的本地命令做 structured policy preflight，从外部 JSON 配置读取 policy profile 行为和 rule-level overrides，支持 `standard` / `strict` profile、`--dry-run` review 和可选 `--signing-key` packet attestation，执行、warning、override、pending review 或拒绝，并生成本地 work packet。
- Wutai 支持在 UI 中导入 CLI wrapper packet 目录或文件，校验 manifest artifact hash，记录 packet provenance，校验可选 packet attestation signature，并用显式加载或本地 enrollment 的 trusted-producer policy 判断 key 是否可信；本地 trust registry 可查看、revoke、reactivate、export key；同时 review policy、trace、ledger、filtered audit、integrity、provenance，并为 dry-run pending execution 记录本地 approve/deny。
- Wutai 支持 v0.4 External Agent Integration Contract：外部 agent/wrapper 可以通过 `sdk/node` 或 `examples/external-agent-wrapper.mjs` 生成 `local_script` packet，Wutai verifier 接受带有 `producer.adapter` 的非 Wutai producer packet，并用 manifest hash、packet provenance、可选 attestation、trusted-producer policy 和 trust-policy profile 产出 `trusted` / `review_required` / `blocked` verdict。
- Wutai 提供 `schemas/` 下的 packet contract JSON schemas、`docs/packet-contract.md` 合同文档、`config/wutai-trust-policy-profiles.json` 下的 `personal-default` / `strict-local` / `ci-review` verifier profile，以及 `.github/workflows/wutai-verify-packet.example.yml` 作为 CI packet gate 示例。
- Wutai 支持 v0.5 Agent Packet Inbox：从本地 task history 派生 packet inbox，按 producer、verdict、retention 和搜索文本过滤，展示 adapter registry、trust verdict、provenance/integrity 状态、attestation 状态和 retained/rejected 本地留存决策。
- Wutai 提供 `config/wutai-adapter-registry.json`、`docs/agent-packet-inbox.md`、`examples/adapter-proof-runner.mjs`，并用 proof harness 生成/验证 `codexCli`、`claudeCode`、`githubActions` 三类 producer packet。
- Wutai 支持 v0.10 Attention Decision Gate：`wutai attention-decision` / `npm run wutai:attention` 重新运行 packet verifier，可选读取 scoped ratification check 和 attention policy，输出 `auto_accepted_under_policy`、`human_attention_required`、`scoped_ratified` 或 `blocked_or_unowned`；输出会拆分 `permissionBasis` 与 `riskSignals`，只有 `mechanical_allowlist` 和 passed `deterministic_external_check` 可作为放行依据，model-backed external check、语义比对和模型判断只能作为 non-granting signal；默认 policy 要求 auto acceptance 前存在 accountable seat，并会在缺 seat 时记录 audit reason。
...
- local-script trace packet 会生成 `manifest.json`、`report.md`、`trace.json`、`audit.json`。
- coding-agent trace packet 会生成 `manifest.json`、`report.md`、`trace.json`、`audit.json`。
- MCP tool-call trace packet 会生成 `manifest.json`、`report.md`、`trace.json`、`audit.json`。
- local file packet 会生成 `manifest.json`、`report.md`、`files.json`、`audit.json`，并可在 re-check 后生成 `file-check.json`。
- CLI wrapper packet 会生成 `manifest.json`、`report.md`、`policy.json`、`trace.json`、`ledger.json`、`audit.json`，并可在 `--signing-key` 下生成 `attestation.json`；UI 导入后会额外保存 `integrity.json` 和 `provenance.json`，dry-run review 决策会额外保存 `review.json`。
- External-agent SDK packet 使用同一个 `local_script` packet contract，生成 `manifest.json`、`report.md`、`policy.json`、`trace.json`、`ledger.json`、`audit.json`，并可在传入 signing key 时生成 `attestation.json`；verifier 可额外写入 `integrity.json`、`provenance.json`、`policy-review.json` 和 `trust-verdict.json`。
- Agent Packet Inbox retention 会生成 `retention.json`，记录 `wutai.packet_retention_decision`，作为本地 retained/rejected 决策 artifact。
- `manifest.json` 会记录 artifact inventory、SHA-256、producer、权限摘要、session/audit 摘要、Evidence Gate 状态或 not-available、coverage 和 blind spots。
...
- Wutai 已经实时监督或沙箱通过 v0.4 contract 产包的外部 agent。
- Wutai 已经能用 model-backed 或 semantic judgment 自动判断 agent work 可以放行。
- Wutai 已经独立证明 attention policy 中声明的 external check 确实是 deterministic。
- Wutai 已经有官方 Codex CLI、Claude Code 或 GitHub Actions live integration。
- Wutai 已经能沙箱任意 shell command。
- Wutai 已经能识别或阻止所有 destructive command。
