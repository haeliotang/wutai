...
| CLI wrapper | `scripts/wutai_run.mjs` | 开发期 local command 执行 ledger |
...
1. 基于当前 CLI wrapper 的 policy preflight。
...
- developer CLI wrapper 会执行一个显式传入的本地命令，生成 `manifest.json`、`report.md`、`trace.json`、`ledger.json` 和 `audit.json`，记录 argv、cwd、exit code、stdout/stderr 摘要和 git-status delta。

下一目标：给 CLI wrapper 增加 policy preflight，同时继续保持 schema 可表示 coding-agent session 和 browser task。
...
- CLI wrapper policy preflight：危险命令 warning、explicit allow/deny、policy artifact
...
- Wutai 支持通过开发期 CLI wrapper 执行显式传入的本地命令，并生成本地 work packet。
...
- CLI wrapper packet 会生成 `manifest.json`、`report.md`、`trace.json`、`ledger.json`、`audit.json`。
...
- Wutai 已经能阻止 destructive command。
